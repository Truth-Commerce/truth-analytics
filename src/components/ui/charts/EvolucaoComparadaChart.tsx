  ...
  return (
    <div>
      <div style={{ width: '100%', height }}>
        <ResponsiveContainer>
          <ComposedChart data={data} margin={{ top: 8, right: 8, bottom: 0, left: 8 }}>
            ...
            {temAnterior ? (
              <Line dataKey="anterior" name="Período anterior" type="monotone" stroke="#6f685f" strokeWidth={1.5} strokeOpacity={0.6} dot={false} connectNulls />
            ) : null}
            ...
            <Line dataKey="media" name="Média móvel 7d" type="monotone" stroke="#267aa5" strokeWidth={1.5} strokeDasharray="5 4" dot={false} />
          </ComposedChart>
        </ResponsiveContainer>
      </div>
      <ul className="mt-3 flex flex-wrap justify-center gap-x-4 gap-y-1.5 text-xs text-muted">
        ...
        <li className="flex items-center gap-1.5"><span aria-hidden="true" className="h-2 w-2 rounded-full" style={{ backgroundColor: '#267aa5' }} />Média móvel 7d</li>
        {temAnterior ? (
          <li className="flex items-center gap-1.5"><span aria-hidden="true" className="h-2 w-2 rounded-full" style={{ backgroundColor: '#6f685f' }} />Período anterior</li>
        ) : null}
      </ul>
