...
export async function createTemplateAction(
  ...
  const templateId = await createTemplate({ ...parsed.data, checklist });
...
export async function updateTemplateAction(
  ...
  if (!parsed.success) return { error: 'Dados inválidos. Confira os campos e tente novamente.' };
  const { id, titulo, tipo, descricao } = parsed.data;
  const checklist = parseChecklist(formData.get('checklist'));

  await updateTemplate(id, { titulo, tipo, descricao, checklist });
